        ...
        clearStorage: function() {
            if (this._storage.clear)
                ...
                for (i in this._storage)
                    this._storage[i].value && this.remove(i);
            return !0
        }
    }
      , c = {
      ...
      , d = {
        ...
        setStorage: function(a, b) {
            this._storage.transaction(function(d) {
                d.executeSql("SELECT v FROM SessionStorage WHERE k = ?", 
                [a], function(d, c) {
        ...
        _createTable: function() {
            this._storage.transaction(function(a) {
                a.executeSql("SELECT COUNT(*) FROM SessionStorage", [], function() {}, function(a, b) {
...
var Memory = (new Class({
    ...
    initialize: function(a, c) {
        ...
        var b = a.indexOf("[");
        this.key = "S";
        -1 < b ? a.lastIndexOf("]") == a.length - 1 ? this.key += (b ? "[" + a.substr(0, b) + "]" : "") + (b ? a.substr(b) : a) : this.key += a : this.key += "[" + a.replace(".", "][") + "]";
    ...
    this.set = function(a, b, c) {
        ...
        "object" == typeOf(b) ? Object.each(b, function(a, b) {
            ...
            Object.each(b, function(a, b) {
                f(h, b).module = a
            }) : "string" == typeOf(b) && (c = a + "_" + b,
            f(h, b).module = c)
        }) : "string" == typeOf(b) && (c = c || a + "_" + b,
        f(h, b).module = c);
        return this
    }
    ;
    ...
    if (0 != e) {
        for (; d < e; d++) {
            ...
            if (0 < d) {
                ...
                0 == b && (g = h.getPosition().y,
                f > g && (b = d))
            }
        }
        0 == b && (b = e);
        d = a[0].getParent().getParent();
        // console.log(d.getPatch().x);
        d = d.getSize().x - d.getPatch().x;
        a[b - 1] && (d += parseInt(a[b - 1].getStyle("margin-right")));
        d = parseInt(d / b);
        a.setStyle("width", d - a[0].getPatch().x);
        d = 0;
        for (e = a.length / b; d < e; d++) {
            var l = a.slice(d * b, b * (d + 1));
            l.length && (l[0].addClass("first"),
            l.getLast().addClass("last"),
            c.each(function(a) {
                setEqualHeight(l.invoke("getElement", a))
            }))
        }
    }
}
function autoGridLayout(a, c, b) {
    a = $$(a);
    if (a.length) {
        if (null  === b || "undefined" === typeof b)
            b = !0;
        c = a[0].getParent(c);
        c = c.getSize().x - c.getPatch("padding", "border").x;
        var d = parseInt(a[0].getStyle("width"))
          , d = a[0].getSize().x || d + a[0].getPatch("padding", "border").x
          , e = parseInt(c / d)
          , g = parseInt((c - e * d) / (e - 1));
        if (b) {
            var f;
            a.each(function(a, b) {
                f = 0 === (b + 1) % e ? 0 : g;
                a.setStyle("margin-right", f)
            })
        } else
            a.setStyle("margin-right", g)
    }
}
function getRowsHeight(a) {
    var c = 0;
    a.each(function(a) {
        a && (a = a.getStyle("height").toInt(),
        a > c && (c = a))
    });
    return c
}
...
function updateCartInfo(a) {
    a = Memory.get("cart_number");
    var c = Memory.get("cart_count")
      , b = Memory.get("cart_total_price");
    try {
        (a || 0 === a) && $$(".op-cart-number").set("text", a),
...
var priceControl = {
    ...
    format: function(a, c) {
        ...
        a = a.round(this.spec.decimals) + "";
        var e = a.indexOf(".");
        0 > e ? (e = a.length,
        b = "") : b = a.substr(e + 1);
        for (; b.length < this.spec.decimals; )
            b += "0";
        for (var g = []; 0 < e; )
            if (2 < e)
                e -= 3,
    ...
    number: function(a) {
        if (!a)
            return null ;
        isNaN(+a) && ("element" === typeOf(a) && (a = a.get("INPUT" === a.tagName ? "value" : "text")),
        0 == a.indexOf(this.spec.sign) && (a = a.split(this.spec.sign)[1]));
        return +a
    },
    calc: function(a, c, b, d) {
        ...
        if (b) {
            ...
            var e = 1
              , g = 1;
            "array" === typeOf(c) && (e = c[1],
            c = c[0]);
            "array" === typeOf(b) && (g = b[1],
            b = b[0]);
            var f = Math.pow(10, this.spec.decimals * 
            this.spec.decimals);
...
function bindDatepicker(a) {
    a || (a = "input.calendar");
    a = $$(a);
    var c = window.Shop ? Shop.url.datepicker : "";
    if (a.length)
        try {
...
window.addEvent("domready", function() {
    ...
    var a = window.Shop ? Shop.url.fav_url : ""
      ...
      , b = function(b, c, d) {
        function e(a) {
            a.innerHTML = '<span class="fav-on">已收藏</span><i>|</i><a href="javascript:void(0);" class="btn-delete">删除</a> '
        }
        if (c && !b.hasClass("fav-on")) {
            var k = b.getParent(".p-action");
            if (d)
                return e(k);
            ...
            (new Request({
                url: a,
                onRequest: function() {
                    this.FAVHTML = k.innerHTML;
                    //k.innerHTML = '<span class="fav-loading">收藏中...</span>'
                },
                onSuccess: function(a) {
                    //(a = JSON.decode(a)) && a.success ? (k.innerHTML = '<span class="fav-success"><q class="icon">&#x25;</q> 收藏成功</span>',
                	//e.delay(2E3, this, k)) : k.innerHTML = this.FAVHTML
                	(a = JSON.decode(a)) && a.success ? (k.innerHTML = '',
                	e.delay(1, this, k)) : k.innerHTML = this.FAVHTML
                }
            })).post({
                type: b,
                gid: c
...
function starGrade(a, c, b) {
    ...
    a && a.getElements(b || ".scores-item").each(function(a) {
        ...
        c || 0 === c ? (b.className = "stars stars-" + c,
        h.set("text", c),
        f.value = c) : (b.addEvent("mouseleave", function() {
            var a = f.value;
